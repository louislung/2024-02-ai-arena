...
contract AiArenaHelper {
    ...
    function createPhysicalAttributes(
        ...
        if (dendroidBool) {
            ...
            uint256[] memory finalAttributeProbabilityIndexes = new uint[](
                attributes.length
            );
            ...
            for (uint8 i = 0; i < attributesLength; i++) {
                if (
                    (i == 0 && iconsType == 2) ||
                    (i == 1 && iconsType > 0) || // Custom icons head (beta helmet) // Custom icons eyes (red diamond)
            ...
            return
                // @note any meaning in those PhysicalAttributes?
    ...
    function addAttributeProbabilities(
        ...
        require(
            ...
            "Invalid number of attribute arrays"
        );

        uint256 attributesLength = attributes.length;
        for (uint8 i = 0; i < attributesLength; i++) {
            // @note any contraint to probabilitie? e.g. sum <= 100
            attributeProbabilities[generation][attributes[i]] = probabilities[
                i
            ];
        }
    }
    ...
    function getAttributeProbabilities(
        uint256 generation,
